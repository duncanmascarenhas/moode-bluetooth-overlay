...
# ==============================================================================
# Moode Audio Premium Bluetooth Overlay (V2.0 - Native Integration)
...
echo "========================================================"
echo " Installing Moode Premium Bluetooth Overlay V2.0"
echo " Utilizing Moode Native Art Rendering (moodeutl & radiocover_plus)"
echo "========================================================"

echo "==> [1/5] Creating native Python D-Bus listener (/usr/local/bin/bluez_art.py)..."
cat << 'PYEOF' > /usr/local/bin/bluez_art.py
import dbus, dbus.mainloop.glib, json, subprocess
...
def fetch_moode_artwork(artist, title):
    ...
    # 2. Trigger Moode's native Radio Cover+ engine
    try:
...
def update_meta(artist, title, album, status="playing"):
    if status == "stopped" or (not title and not artist):
        with open(META_PATH, 'w') as f:
            json.dump({"title": "Playback Stopped" if status == "stopped" else "No Track Playing", 
                       "artist": "", "album": "", "cover_url": DEFAULT_COVER}, f)
        return
        
    cover_url = fetch_moode_artwork(artist, title)
    with open(META_PATH, 'w') as f:
        json.dump({"title": title, "artist": artist, "album": album, "cover_url": cover_url}, f)
...
def on_property_changed(interface, changed, invalidated, path):
    ...
    if "Track" in changed:
        t = changed["Track"]
        update_meta(str(t.get("Artist", "")), str(t.get("Title", "")), str(t.get("Album", "")))
...
echo "--------------------------------------------------------"
echo " V2 Installation Complete! Please hard refresh your browser."
echo "--------------------------------------------------------"
